...
export function resolveSrcDir(srcDir: string | undefined, config: ServerConfig): string {
    ...
    if (!srcDir) {
        ...
        throw new Error(
            'No source directory specified. Provide srcDir parameter or configure --src-dir'
        );
    }
...
export async function validatePath(
    ...
    try {
        ...
        if (error instanceof Error && 'code' in error && error.code === 'ENOENT') {
            throw new Error(
                `${pathType === 'directory' ? 'Directory' : 'File'} not found: ${resolvedPath}`
            );
        }
        throw error;
    }
 ...
 */
export function createPathDescription(
