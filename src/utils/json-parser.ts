...
export class JsonParser {
    ...
    static async parseFile(filePath: string): Promise<ParseResult> {
        try {
            ...
            throw new Error(
                `Failed to parse JSON file ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
        }
    }
    ...
    static validateStructure(data: any, template: any): { valid: boolean; errors: string[] } {
        ...
        const validate = (current: any, templateCurrent: any, path: string = '') => {
            if (typeof templateCurrent !== typeof current) {
                errors.push(
                    `Type mismatch at ${path}: expected ${typeof templateCurrent}, got ${typeof current}`
                );
                return;
            }
            ...
            if (
                templateCurrent &&
                typeof templateCurrent === 'object' &&
                !Array.isArray(templateCurrent)
            ) {
